...
export default class GM3Level3 extends BaseGM3Scene {
  ...
  preload() {
    this.load.binary("gm3_hard_xlsx", "assets/UpdatedAccountingElements.xlsx");
  ...
  _finishToGameOver(reason = "completed") {
    ...
    this.scene.start("GameOverScene", { score: this.score, mode: "GM3-Level3", reason });
  }
  ...
  buildLevel() {
    ...
    try {
      ...
      const sheetName = wb.SheetNames.find(n => n.trim().toLowerCase() === "a=l+se - hard".toLowerCase());
      if (!sheetName) return this._failAndBack("Sheet 'A=L+SE - Hard' not found.");
      ...
      const normalizeCode = (cell) => {
        ...
        if (t === "NE" || t.includes("NO EFFECT") || t === "NONE") return "NE";
        if (t === "U" || t === "UP" || t === "INCREASE" || t === "INC" || t === "RISE") return "U";
        if (/[+\uFF0B]/.test(t) || t.includes("PLUS") || t.includes("POSITIVE")) return "U";
        if (/[-\u2212\u2012\u2013\u2014\u2015]/.test(t) || t.includes("MINUS") || t.includes("NEGATIVE")) return "O";
        return "NE";
      };
      ...
      for (let r = 4; r <= MAX_SCAN_ROWS; r++) {
        ...
        const rowAllBlank = (!q && asset === "NE" && liab === "NE" && se === "NE" && ni === "NE");
    ...
    this.plusText = this.add.text(this.plusTextAnchor.x, this.plusTextAnchor.y, "+300", {
      fontSize: "48px",
      color: "#dcc89f",
      fontFamily: '"Jersey 10", sans-serif',
  ...
  _onSubmit() {
    ...
    const cor = item.correct ?? { Asset: "NE", Liability: "NE", SE: "NE", NI: "NE" };
