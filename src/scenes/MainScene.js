...
export class MainScene extends Scene {
    ...
    init(data) {
        ...
        this.answer_stats = new Map(
            this.config.basket_types.map((type) => [type, { correct: 0, incorrect: 0 }])
        );
    ...
    create() {
        ...
        belts_chosen.forEach((belt_label) => {
            ...
            while (belt_num < num_belts - 2) {
                this.conveyor_belts.push(new ConveyorBelt(this));
                let [bx, by] = get_pos_from_belt_and_num(this, belt_label, belt_num);
                this.conveyor_belts[this.conveyor_belts.length - 1].set_pos_and_belt_label(bx, by, belt_label);
                belt_num++;
            }

            let [basket_x, basket_y] = get_pos_from_belt_and_num(this, belt_label, belt_num);
        ...
        this.conveyor_belts.forEach((belt) => {
            if ([1, 2, 3].includes(belt.belt_label)) this.children.bringToTop(belt);
        });
        ...
        const move_along_conveyor_belt = (scene, conveyor_belt, obj) => {
            ...
            if (conveyor_belt.belt_label === 2 || conveyor_belt.belt_label === 3) {
        ...
        this.physics.add.overlap(this.conveyor_belts, this.balls, (belt, ball) => {
            if (ball.state !== "picked") {
                if (ball.direction_belt_label == null) ball.direction_belt_label = belt.belt_label;
                if (belt.belt_label == ball.direction_belt_label) move_along_conveyor_belt(this, belt, ball);
            }
        });
    ...
    update(time, delta) {
        ...
        // --- Input: pick/drop ---
        if (Phaser.Input.Keyboard.JustDown(this.keySpace) || (this.input.activePointer.leftButtonDown() && !this.mouse_down_last_frame)) {
            if (this.player.ball && this.player.ball.state === "picked") {
                ...
                this.balls.getChildren().forEach((ball) => {
                    ...
                    if (Phaser.Geom.Intersects.RectangleToRectangle(ball.getBounds(), this.player.getBounds())) {
