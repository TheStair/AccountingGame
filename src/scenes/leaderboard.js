  ...
  async loadLeaderboard(mode, button = null) {
    ...
    try {
      ...
      data.forEach((entry, i) => {
        ...
        const rankText = this.add.text(rankX, y, `${i + 1}.`, { fontSize: '18px', fill: '#ffffff' }).setOrigin(0, 0);
        const nameText = this.add.text(nameX, y, entry.username, { fontSize: '18px', fill: '#ffffff' }).setOrigin(0, 0);
        const scoreText = this.add.text(scoreX, y, entry.score.toString(), { fontSize: '18px', fill: '#ffffff' }).setOrigin(1, 0);
