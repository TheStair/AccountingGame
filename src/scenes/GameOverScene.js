...
export class GameOverScene extends Scene {
  ...
  init(data) {
    ...
    // - GM3Level1/2 currently send: { score, mode: "GM3-Level1"/"GM3-Level2" }
    // - Other callers might send: { points, gameKey }
  ...
  async create() {
    ...
    try {
      // Use the same "game" identifier we were given (mode or gameKey)
      const previewRes = await fetch(
        `${this.game.apiBaseUrl}/preview?game=${encodeURIComponent(this.gameKey)}&score=${this.end_points}`
      );
