...
export class PauseScene extends Scene {
    ...
    create() {
        ...
        // --- Background bands ---
        const firstRectangle = this.add.rectangle(
            0,
        ...
        const secondRectangle = this.add.rectangle(
            0,
            height / 2 +
        ...
        const primary_click = this.add.image(
            (width * 3) / 4 + 160,
            height / 2 + 30 + vertical_shift_to_center,
            "primary_click"
        ).setScale(0.15);

        const space_bar = this.add.image(
            (width * 3) / 4 - 90,
            height / 2 + 30 + vertical_shift_to_center,
            "space_bar"
        ).setScale(0.5);

        const pickup_controls_label = this.add.text(
            (width * 3) / 4,
        ...
        const pickup_controls_or = this.add.text(
            (width * 3) / 4 + 90,
            height / 2 + 30 + vertical_shift_to_center,
            "or",
            {
        ...
        const WASD = this.add.image(
            width / 4 - 100,
            height / 2 + 30 + vertical_shift_to_center,
            "WASD"
        ).setScale(0.5);

        const arrow_keys = this.add.image(
            width / 4 + 100,
            height / 2 + 30 + vertical_shift_to_center,
            "arrow_keys"
        ).setScale(0.5);

        const movement_controls_label = this.add.text(
            width / 4,
            height / 2 - 50 + vertical_shift_to_center,
            "Movement:",
            {
        ...
        const movement_controls_or = this.add.text(
            width / 4,
            height / 2 + 30 + vertical_shift_to_center,
            "or",
            {
    ...
    createStyledButton(x, y, label, callback) {
        ...
        container.on("pointerover", () => {
            ...
            this.tweens.add({ targets: container, scale: 1.05, duration: 150, ease: "Power1" });
        });
